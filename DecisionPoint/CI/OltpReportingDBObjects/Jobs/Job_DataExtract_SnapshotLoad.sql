DECLARE  @ServerName VARCHAR(255) = '"\"$(servername_)\""'
		...
		,@DataExtractTypeId INT = 2
		,@FileSize INT = 2000
		,@SSISPackagePath VARCHAR(MAX) = '"\"\SSISDB\AcsOds_Extract\AcsOds\Data File Extracts.dtsx\""'
		,@Command VARCHAR(MAX);

DECLARE @JobName VARCHAR(MAX);
SET @JobName = N'DW Data Extracts - '+DB_NAME()+' - Snapshot Load'
...
BEGIN
	...
	EXEC @ReturnCode =  msdb.dbo.sp_add_job @job_name=@JobName, 
			...
			@notify_level_page=0, 
			@delete_level=0, 
			@description=N'This Job Dumps Snapshot Data from Tables in rpt.Process into flat files', 
			@category_name=N'Failover', 
			@owner_login_name=N'sa', @job_id = @jobId OUTPUT
	IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback

	EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'AcsOds', 
			@step_id=1, 
