...
AS
BEGIN
-- DECLARE @ReportURL VARCHAR(MAX) = 'http://midashboards.mitchell.com/#/views/ODSLoadResults_0/Dashboard-OverallODSResults',@recipients_param VARCHAR(100) = 'theodore.bofia@mitchell.com',@Rundate DATETIME = '07/27/2018';
...
FROM (SELECT DISTINCT PR.CustomerName
	  FROM adm.ProcessReconciliation PR) AS S
...
SET @tableHTML = REPLACE(@tableHTML,'Header1Text','AcsOds Job Status: Run Completed.'); 
...
@subject = 'AcsOds Load Status Update.',
@body = @tableHTML,
@body_format = 'HTML' ;
END
