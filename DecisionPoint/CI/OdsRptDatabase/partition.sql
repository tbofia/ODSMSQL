...
IF( EXISTS( SELECT * FROM sys.partition_functions WHERE name = 'rpt_PartitionFunction' ) )
BEGIN TRY 
DROP PARTITION FUNCTION rpt_PartitionFunction;
END TRY
BEGIN CATCH
...
DECLARE @IntegerPartitionFunction nvarchar(max) = N'CREATE PARTITION FUNCTION rpt_PartitionFunction (int) AS RANGE LEFT FOR VALUES (';
DECLARE @i int = 1;
WHILE @i < 100
BEGIN
...
WHERE f.name = 'rpt_PartitionFunction'
*/

-- Partition Scheme 
IF( EXISTS( SELECT * FROM sys.partition_schemes WHERE name = 'rpt_PartitionScheme' ) )
BEGIN TRY
 DROP PARTITION SCHEME rpt_PartitionScheme;
END TRY
BEGIN CATCH
...
BEGIN TRY
CREATE PARTITION SCHEME rpt_PartitionScheme
    AS PARTITION rpt_PartitionFunction
    ALL TO ('PRIMARY');
END TRY
BEGIN CATCH
...
WHERE ps.name = 'rpt_PartitionScheme'

*/

-- Use This Part when need to split partitions e.g. when adding a new customer
/*
ALTER PARTITION SCHEME rpt_PartitionScheme 
NEXT USED 'PRIMARY';

ALTER PARTITION FUNCTION rpt_PartitionFunction ()
