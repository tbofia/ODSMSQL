...
EXEC @ReturnCode =  msdb.dbo.sp_add_job @job_name=N'RPT: VPN Monitoring Report', 
		@enabled=1, 
...
EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'VPN_Monitoring_Initialize', 
		@step_id=1, 
...
EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'VPN_Monitoring_NetworkRepricedSubmittedCredits', 
		@step_id=2, 
		@cmdexec_success_code=0, 
		@retry_attempts=0, 
		@retry_interval=0, 
...
EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'VPN_Monitoring_TAT', 
		@step_id=3, 
		@cmdexec_success_code=0, 
		@retry_attempts=0, 
		@retry_interval=0, 
...
EXEC dbo.VPN_Monitoring_TAT  @SourceDatabaseName,@StartDate,@EndDate,@if_Date,@RunType,@OdsCustomerId;
...
EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'VPN_Monitoring_GreenwichData', 
		@step_id=4, 
		@cmdexec_success_code=0, 
		@retry_attempts=0, 
		@retry_interval=0, 
		@os_run_priority=0, @subsystem=N'TSQL', 
		@command=N'EXEC dbo.VPN_Monitoring_GreenwichData', 
		@database_name=@DatabaseName, 
		@flags=0
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback

EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'VPN_Monitoring_CloseTrackingSendNotification', 
		@step_id=5, 
		@cmdexec_success_code=0, 
		@on_success_action=1, 
