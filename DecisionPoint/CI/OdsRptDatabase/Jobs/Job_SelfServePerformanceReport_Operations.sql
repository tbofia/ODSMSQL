...
EXEC @ReturnCode =  msdb.dbo.sp_add_job @job_name=N'RPT: Self Serve Performance Report Operations', 
		@enabled=1, 
...
EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'Self Serve Performance Report Operations Initialize Tables', 
		@step_id=1, 
...
EXEC adm.Mnt_TrackCustomerReport @ReportId,NULL,0,@SourceDatabaseName,NULL,NULL;', 
		...
		@flags=0
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'Self Serve Performance Report Operations Output', 
		@step_id=2, 
		@cmdexec_success_code=0, 
		@on_success_action=3, 
		@on_success_step_id=0, 
...
EXEC dbo.SelfServePerformanceReport_Operations_Output  @SourceDatabaseName, @StartDate, @EndDate, @if_Date, @RunType, @OdsCustomerID;
...
EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'Self Serve Performance Report Operations_CloseTrackingSendNotification', 
		@step_id=3, 
		@cmdexec_success_code=0, 
		@on_success_action=1, 
		@on_success_step_id=0, 
