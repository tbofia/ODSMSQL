...
EXEC @ReturnCode =  msdb.dbo.sp_add_job @job_name=N'RPT: Loss Year Report', 
		@enabled=1, 
...
EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'LossYearReport_Initialize', 
		@step_id=1, 
...
EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'LossYearReport_Data', 
		@step_id=2, 
		@cmdexec_success_code=0, 
		@retry_attempts=0, 
		@retry_interval=0, 
...
SET @StartDate = DATEADD(YEAR,DATEDIFF(YEAR,0,@EndDate)-5,0);
...
EXEC dbo.LossYearReport_Input  @SourceDatabaseName, @StartDate, @EndDate,@if_Date, @RunType, @OdsCustomerID,@ReportId,@ProcessId;
...
EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'LossYearReport_Extract Client', 
		@step_id=3, 
		@cmdexec_success_code=0, 
		@retry_attempts=0, 
		@retry_interval=0, 
...
IF @OdsCustomerId <> 0
DELETE FROM stg.LossYearReport_Client
WHERE OdsCustomerID = @OdsCustomerId
ELSE 
TRUNCATE TABLE stg.LossYearReport_Client;
 
EXEC dbo.LossYearReport_Filtered   @SourceDatabaseName, @if_Date,@RunType,@OdsCustomerId
EXEC dbo.LossYearReport_Client   @SourceDatabaseName, @if_Date,@RunType,@OdsCustomerId,0
EXEC dbo.LossYearReport_Client   @SourceDatabaseName, @if_Date,@RunType,@OdsCustomerId,1
...
EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'LossYearReport_Extract Industry', 
		@step_id=4, 
		@cmdexec_success_code=0, 
		@retry_attempts=0, 
		@retry_interval=0, 
		@os_run_priority=0, @subsystem=N'TSQL', 
		@command=N'DECLARE @SourceDatabaseName VARCHAR(50) 

SELECT @SourceDatabaseName = SourceDatabaseName
FROM adm.ReportJob
WHERE ReportJobName = ''$(ESCAPE_SQUOTE(JOBNAME))'';


EXEC dbo.LossYearReport_Industry @SourceDatabaseName
...
EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'LossYearReport_Output', 
		@step_id=5, 
		@cmdexec_success_code=0, 
		@retry_attempts=0, 
		@retry_interval=0, 
...
DECLARE db_customer_cursor CURSOR FOR 
SELECT C.CustomerId FROM @CustomerList C
INNER JOIN rpt.CustomerReportInclusion E
ON C.CustomerId = E.CustomerId
AND E.ReportID = @ReportId
...
EXEC dbo.LossYearReport_Output @SourceDatabaseName, @RunType,@OdsCustomerId,@ReportId, @ProcessId;
...
EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'LossYearReport_Greenwich Data', 
		@step_id=6, 
		@cmdexec_success_code=0, 
		@retry_attempts=0, 
		@retry_interval=0, 
		@os_run_priority=0, @subsystem=N'TSQL', 
		@command=N'EXEC dbo.LossYearReport_GreenwichData', 
		@database_name=@DatabaseName, 
		@flags=0
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback

EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'LossYearReport_CloseTrackingSendNotification', 
		@step_id=7, 
		@cmdexec_success_code=0, 
		@on_success_action=1, 
