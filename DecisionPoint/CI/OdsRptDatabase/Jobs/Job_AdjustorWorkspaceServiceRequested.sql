...
EXEC @ReturnCode =  msdb.dbo.sp_add_job @job_name=N'RPT: Adjuster Workspace Service Requested Report', 
		@enabled=1, 
		...
		@notify_level_page=0, 
		@delete_level=0, 
		@description=N'(Job owner changed from [CORP\tb101541] on 2017-06-29 at 23:30:02.)', 
		@category_name=N'Failover', 
		@owner_login_name=N'sa', @job_id = @jobId OUTPUT
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback

EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'AdjustorWorkspaceServiceRequested_CleanUp', 
		@step_id=1, 
...
-- Get report Name
SET @SQLScript = ''SELECT @ReportName = ReportName FROM ''+@SourceDatabaseName+''.dbo.BillExclusionLookUpTable WHERE ReportID = ''+CAST(@ReportId AS VARCHAR(3));
...
WHERE TABLE_TYPE=''BASE TABLE''
	AND TABLE_NAME LIKE REPLACE(@ReportName,'' '',''_'')+''Report_Tracking%'';
...
EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'AdjustorWorkspaceServiceRequested_Package', 
		@step_id=2, 
		@cmdexec_success_code=0, 
		@retry_attempts=0, 
		@retry_interval=0, 
...
EXEC dbo.AdjustorWorkspaceDemandPackage @SourceDatabaseName,@if_Date,@RunType,@OdsCustomerId;
...
EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'AdjustorWorkspaceServiceRequested_ServiceRequested', 
		@step_id=3, 
		@cmdexec_success_code=0, 
		@retry_attempts=0, 
		@retry_interval=0, 
...
EXEC dbo.AdjustorWorkspaceServiceRequested @SourceDatabaseName,@if_Date,@RunType,@OdsCustomerId;
...
EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'AdjustorWorkspaceServiceRequested_CloseTrackingSendNotification', 
		@step_id=4, 
		@cmdexec_success_code=0, 
		@on_success_action=1, 
...
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
EXEC @ReturnCode = msdb.dbo.sp_add_jobschedule @job_id=@jobId, @name=N'Weekly Run', 
		@enabled=1, 
		@freq_type=8, 
		@freq_interval=1, 
		@freq_subday_type=1, 
		...
		@schedule_uid=N'508aa3c4-75a7-45db-86ba-9f072707eecb'
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
