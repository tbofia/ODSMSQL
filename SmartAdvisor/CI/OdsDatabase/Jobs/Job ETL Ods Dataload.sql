...
DECLARE  @ServerName VARCHAR(255) = '"\"$(servername_)\""'
		,@SSISPackagePath VARCHAR(MAX) = '"\"\SSISDB\WcsOds_DataLoad\SA ODS Loading Project\ODSLoad_Master.dtsx\""'
		,@Command VARCHAR(MAX);
...
WHERE JobRunId = @LastRunId', 
		...
		@flags=0
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [Kick Off Snowflake Replication Incremental Load]    Script Date: 12/4/2020 10:46:30 AM ******/
EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'Kick Off Snowflake Replication Incremental Load', 
		...
		@retry_interval=0, 
		@os_run_priority=0, @subsystem=N'TSQL', 
		@command=N'IF CAST(GETDATE() AS TIME) >= ''08:00:00'' EXEC dbo.sp_start_job N''SnowFlake Replication: Incremental Load''
GO', 
